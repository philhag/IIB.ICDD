...
namespace IIB.ICDD.Model.ExtendedLinkset
{
    ...
    /// Class:  LsExtElaborates 
    /// (c) 2022 Philipp Hagedorn, Chair of Computing in Engineering, Ruhr-University Bochum, Germany
    /// Mail: philipp.hagedorn-n6v@rub.de 
    /// </summary>
    /// <seealso cref="LsDirected1ToNLink" />
    public class LsExtElaborates : LsDirected1ToNLink
    {
        internal LsExtElaborates(CtLinkset linkset, LsLinkElement fromElement, List<LsLinkElement> toElements) :
            base(linkset, fromElement, toElements)
        {
        }

        protected LsExtElaborates(CtLinkset linkset, INode resource) : base(linkset, resource)
        {
        }

        public new static LsExtElaborates Read(CtLinkset linkset, INode resource)
        {
            try
            {
                return new LsExtElaborates(
        ...
        {
            string s = "LsExtElaborates: " + FromElement + "-->";
        ...
        {
            return IcddTypeSpecsHelper.ExLsElaborates;
        }

        public override IUriNode Type()
        {
            return Graph.CreateUriNode(IcddTypeSpecsHelper.ExLsElaborates);
        }
    }
}
